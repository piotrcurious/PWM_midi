...
// Constants
const int analogPinError = A0;  // Analog input pin for error value
const int analogPinBaseFreq = A1; // Analog input pin for base frequency
...
int mapErrorToNoteInChord(int errorValue, int* chord, int chordSize, int baseNote);
void sendMIDINoteOn(int note);
void sendMIDINoteOff(int note);
int predictError(int currentError);
void playChordProgression(int errorValue, int baseNote);
void visualFeedback(int intensity);
void sendChord(int* chord, int chordSize, int baseNote);
...
void loop() {
  // Read the current error value and base frequency
  int currentError = readErrorValue();
  int baseFrequency = readBaseFrequency();
  
  // Map the base frequency to a MIDI note
  int baseNote = map(baseFrequency, 0, 1023, 48, 72); // Map to a reasonable MIDI note range (C3 to C5)
  Serial.print("Base Note: ");
  Serial.println(baseNote);

  // Play a chord progression based on the current error and base note
...
// Function to read the analog input for error value
int readErrorValue() {
  int sensorValue = analogRead(analogPinError);
...
// Function to read the analog input for base frequency
int readBaseFrequency() {
  int sensorValue = analogRead(analogPinBaseFreq);
  Serial.print("Base Frequency Sensor Value: ");
  Serial.println(sensorValue);
  return sensorValue;
}

// Function to map an error value to a note within a chord, harmonized to the base note
int mapErrorToNoteInChord(int errorValue, int* chord, int chordSize, int baseNote) {
  int index = errorValue % chordSize;
  int midiNote = chord[index] + baseNote; // Harmonize with base note
...
// Function to play a chord progression based on the error value and base note
void playChordProgression(int errorValue, int baseNote) {
