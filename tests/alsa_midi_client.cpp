...
typedef int (*snd_seq_client_id_t)(snd_seq_t *);
...
static snd_seq_client_id_t p_snd_seq_client_id;
...
bool ALSAMIDIClient::open(const std::string& clientName) {
    ...
    p_snd_seq_client_id = (snd_seq_client_id_t)dlsym(handle, "snd_seq_client_id");

    if (!p_snd_seq_open || !p_snd_seq_close || !p_snd_seq_set_client_name ||
        !p_snd_seq_create_simple_port || !p_snd_seq_connect_to || !p_snd_seq_event_output_direct || !p_snd_seq_client_id) {
        return false;
    }
...
void ALSAMIDIClient::sendNoteOn(int channel, int note, int velocity) {
    ...
    ev.flags = 0;
    ev.source.client = (unsigned char)p_snd_seq_client_id((snd_seq_t*)seq);
    ev.source.port = (unsigned char)port;
    ev.dest.client = 254; // subscribers
    ev.dest.port = 254;
    ev.queue = 253; // direct
